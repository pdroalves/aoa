...
__host__ double runRescale(){
  ...
  for(int i = 0; i < NITERATIONS; i++)
    ct.push_back(ckks_encrypt(cipher, &val));
...
std::vector<T> intersection(std::vector<T> &v1,
 ...
 int main(int argc, char* argv[]){
  ...
  // Command line parser //
  ////////////////////////
  cxxopts::Options options("aoadgt_benchmark-residues", "This program benchmarks the main procedures of SPOG-CKKS");
  options.add_options()
  ("p,procedure", "define which procedure should be measured", cxxopts::value<std::vector<std::string>>())
  ("d,degree", "for which ring degree it should measure the latencies", cxxopts::value<int>())
  ("g,gpu", "which parameter set should be used", cxxopts::value<std::string>())
  ("a,arange", "arange", cxxopts::value<int>())
  ("b,brange", "brange", cxxopts::value<int>())
  ("c,pace", "pace", cxxopts::value<int>())
  ...
  int brange = 10;
  int pace = 5;
  if (result.count("arange"))
    arange = result["arange"].as<int>();
  if (result.count("brange"))
    brange = result["brange"].as<int>();
  if (result.count("pace"))
    pace = result["pace"].as<int>();
  ...
  for( int r = arange; r < brange; r += pace){
    // Init
    // 
    // Values:
    // 
    ...
    if (result.count("degree"))
      nphi = result["degree"].as<int>();
    else
      throw std::runtime_error("I need a degree");
    int k = r;
    int kl = k+1;
    ...
    // Start the engine
    cudaEventRecord(start);
    CUDAEngine::init(k, kl, nphi, scalingfactor);
    cudaEventRecord(stop);
    cudaCheckError();
    cudaEventSynchronize(stop);
    cudaCheckError();
    ...
    std::cout << nphi << " (" << NTL::NumBits(CUDAEngine::RNSProduct) << " bits)" << std::endl;
    print_memory_usage();
    // Tests
  ...
  for( int r = arange; r < brange; r += pace)
    std::cout << r << std::setw(nameWidth) << std::setfill(separator);
  std::cout << std::endl;
